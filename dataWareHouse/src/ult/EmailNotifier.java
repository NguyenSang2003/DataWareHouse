...
//	gửi email khi không có dữ liệu
	...
	//	gửi email khi không thể ghi log
	public void sendErrorLog(String errorMessage) {
		// Cấu hình thông tin cho SMTP server
		Properties props = new Properties();
		props.put("mail.smtp.host", "smtp.gmail.com");
		props.put("mail.smtp.port", "587");
		...
		try {
			...
			message.setFrom(new InternetAddress(senderEmail, "Southern Vietnam Lottery Results"));
			message.setRecipients(Message.RecipientType.TO, InternetAddress.parse(recipientEmail));
			message.setSubject("Error in Logging to Database");

			String htmlContent = "<html><body>" + "<h2 style='color:#ff0000;'>Error in Logging to Database</h2>"
					+ "<p><strong>Error Message:</strong></p>"
					+ "<p><i>" + errorMessage + "</i></p>"
					+ "<p>Please check the system and investigate the issue.</p>" + "</body></html>";
